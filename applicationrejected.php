...
<div class="container my-4">
    <h2 class="text-center mb-4">Rejected Applications</h2>

    <!-- Summary Section -->
    <div class="summary">
        <div class="card">
            <h3>Total Rejected Applications</h3>
            <p>
                <?php
                $rejectedCountQuery = "SELECT COUNT(*) AS total_rejected FROM bhapplication WHERE status = 'REJECTED'";
                $rejectedCountResult = mysqli_query($conn, $rejectedCountQuery);
                echo mysqli_fetch_assoc($rejectedCountResult)['total_rejected'];
                ?>
            </p>
        </div>
    </div>
